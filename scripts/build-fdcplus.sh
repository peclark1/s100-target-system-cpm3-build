...
cp "$BUILD/CPM3.SYS" "$DIST/CPM3-FDCPLUS.SYS"
cp "$BUILD/BIOS3.SPR" "$DIST/BIOS3-FDCPLUS.SPR"
cp "$BUILD/BIOS3.SYM" "$DIST/BIOS3-FDCPLUS.SYM"

sys_sha="$(sha256sum "$DIST/CPM3-FDCPLUS.SYS" | awk '{print $1}')"
sys_size="$(stat -c %s "$DIST/CPM3-FDCPLUS.SYS")"

echo
echo "SUCCESS: built experimental Dual-CF + FDC+ CP/M 3"
printf 'CPM3-FDCPLUS.SYS: %s bytes\nSHA256: %s\n' "$sys_size" "$sys_sha"
echo "NOTE: this build is not the hardware-tested gold build yet."
